...
namespace DeviceReader.Functions
{
    public class PersistReadings
    {
        ...
        public PersistReadings(ILogger<PersistReadings> logger, IConfiguration configuration, CosmosClient cosmosClient)
        {
            ...
            _container = _cosmosClient.GetContainer(_configuration["DatabaseName"], _configuration["WriteContainer"]);
        }

        [FunctionName(nameof(PersistReadings))]
        public async Task Run([EventHubTrigger("readings", Connection = "EventHubConnection")] EventData[] events)
        {
